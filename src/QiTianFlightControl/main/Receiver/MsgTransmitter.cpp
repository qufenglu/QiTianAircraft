...
int32_t MsgTransmitter::Init()
{
	...
	//计算控制消息buff长度
	m_nControlMsgbuffLen = 1 + CHANNEL_NUM * 2 + 2;
...
{
	Info("[%p][MsgTransmitter::MakeClientSocket] Enter Connect2Server()\n", this);
...
int32_t MsgTransmitter::UnpackControlMsg(uint8_t* buff,const uint8_t len)
{
	if(len != m_nControlMsgbuffLen)
	{
		Error("[%p][MsgTransmitter::UnpackControlMsg] ControlMsgbuff len:%d err", this,m_nControlMsgbuffLen);
		return -1;
	}
	...
	memcpy(&checksum_cal, buff+(1 + 2 * CHANNEL_NUM), sizeof(uint16_t));
	if(checksum_cal != checksum)
	{
		Error("[%p][MsgTransmitter::UnpackControlMsg] Checksum failed", this);
		return -2;
	}*/
	...
	//MsgContent
	{
		...
		{
			memcpy(&m_ControlMsg.chValues[i], buff+(1 + 2 * i), sizeof(uint16_t));
		}
	}
...
int32_t MsgTransmitter::GetControlMsg(ControlMsg &controlMsg)
{
	{
		std::lock_guard<std::mutex> lock(m_ControlMsgLock);
		memcpy(&controlMsg, &m_ControlMsg, sizeof(ControlMsg));
	}
...
void MsgTransmitter::wifi_event_handler(void *arg, esp_event_base_t event_base, int32_t event_id, void *event_data)
{
...
{
	Info("[MsgTransmitter::InitWifiAsSoftAP] Enter InitWifiAsSoftAP(ssid:%s password:%s)\n", ssid, password);
...
{
	Info("[MsgTransmitter::ConnectToAP] Enter ConnectToAP(ssid:%s password:%s)\n", ssid, password);
